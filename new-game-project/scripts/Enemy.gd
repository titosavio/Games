...
func _draw():
    ...
    var start_angle := atan2(facing_dir.y, facing_dir.x) - half
    var end_angle := atan2(facing_dir.y, facing_dir.x) + half
    ...
    for i in range(num_cone_segments + 1):
        ...
        points.append(Vector2(cos(a), sin(a)) * follow_radius)

    # contorno
    for i in range(points.size() - 1):
        draw_line(points[i], points[i + 1], Color(1, 1, 0, 0.9), 2.0)
